	...
	render() {
		...
		return (
			<div>
				...
				<p>We're going to start writing some HTML in the next step, but first we're going to need to set up your computer so you can work like a real coder. This should take just a couple of minutes.</p>

				<p>First, open up your terminal. You can do that by pressing Command+Spacebar and typing in "Terminal", then pressing enter. The terminal is a program that allows you to type in commands to your computer. It's pretty handy.</p>

				<p>Now we're going to set you up with a text editor so you can write code easily. We'll do that with Sublime Text. First, we'll install a package manager called "Brew". Brew will come in handy for installing Sublime Text, and for a lot of other packages later. To install Brew, type the following. (But don't type the "> " at the beginning. That just means this is something you should run in the terminal.)</p>
				...
				<code>> brew install caskroom/cask/brew-cask && brew tap caskroom/versions && brew cask install sublime-text3</code>
				...
				<code>> git clone https://github.com/{login}/{repoName}.git</code>

				<p>This will create a directory in your home directory called {repoName}. Next, change directories to go into your repository:</p>
				...
				<p>There's not a lot happening in there for right now, but we're going to make some stuff happen right away. Create a new branch to do your coding in. We'll explain later what this step is all about.</p>

				<code>> git checkout -b {branchName}</code>

				<p>Let's create a test file to make sure that you're set up correctly:</p>

				<code>> touch test-file.txt</code>

				<p>Then, go into Sublime Text and click on "test-file.txt" in the left hand side. This will open up the file for editing in the main window. Type "This is just a test" in the file and type Command+S to save.</p>

				<p>Last but not least, send the code to our shared repository by typing</p>

				<code>> git add . && git commit -m "Create my first text file" && git push -u origin {branchName}</code>
			</div>
		);
	}
});
